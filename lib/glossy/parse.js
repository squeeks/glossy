 ...
 *
 *	@param {String} rawMessage Raw message received from socket
...
module.exports.parse = function(rawMessage, callback) {
	...
	// Try RFC 5424
	var messageHead = rawMessage.match(/^(<\d+>\d)\s(\S+)\s(\S+)\s(\S+)\s(.*)$/); //TODO This regex could be tighter

	if(messageHead) {
		var payload = messageHead[5].match(/\s?(\S+|-)\s(\S+|-)\s(\[.*\]\s)?-?(.+)$/);
		if(payload) {
			if(payload[1] != '-') parsedMessage['procID']  = payload[1];
			if(payload[4] != '-') parsedMessage['message'] = payload[4];
			if(payload[3]) parsedMessage['structuredData'] = parseStructure(payload[3]);
		}
	...
	if(messageHead) {
		...
		if(!parsedMessage['message']) parsedMessage['message'] = messageHead[4];
	}
 ...
 *	@param {String} msg The section of 
 *	@return {Object} 
 */
function parseStructure(msg) {
	...
	for(element in sdElements) {
		...
		var sdID = element.match(/^(\w+@\d+)\s/);
		element  = element.replace(sdID[0], '');
		var keys = element.split(/\s/);
		for(key in keys) {
			var sdKeyValue = keys[key].match(/^(.*)="(.*)"$/);
			if(sdKeyValue) {
				var sdKey      = sdKeyValue[1];
				var sdValue    = sdKeyValue[2];
				if(!sdStructure[sdID[1]]) sdStructure[sdID[1]] = { };
				sdStructure[sdID[1]][sdKey] = sdValue;
			}
		}
	}
