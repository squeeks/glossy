...
var SyslogProduce = function(messageFormat) {
	if(typeof messageFormat == 'string') {
		this.messageFormat = messageFormat.match(/bsd|3164/i) ? "RFC3164" : "RFC5424";
	}
	else {
		this.messageFormat = "RFC5424";
	}
};
...
SyslogProduce.prototype.produce = function(options, callback) {
	...
	} else if(this.messageFormat == 'RFC3614') {
	...
	var compiledMessage = msgData['prival'] +' '+ msgData['timestamp'] +' ';
...
function generateDate(dateObject) {
	...
	formattedDate = dateObject.getUTCFullYear()         + '-' +
			...
			leadZero(dateObject.getUTCDate())   + '-' +
			'T' + // Time
