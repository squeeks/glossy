...
GlossyParser.prototype.parse = function(rawMessage, callback) {
    ...
    //some implementations add a null to the end
    var lastPos = parsedMessage.message.length-1;
    if(0x00 === parsedMessage.message.charCodeAt(lastPos)) {
      parsedMessage.message = parsedMessage.message.substring(0,lastPos);
    }
