...
module.exports.parse = function(rawMessage, callback) {
	...
	messageHead   = rawMessage.match(/^(.*) -/);
	headBreak     = messageHead[0].split(' ');
	...
	// FIXME: Have to do a better job than this
	if(headBreak.length == 7)
	{
		priKey = decodePri(headBreak[0]);
		for (key in priKey)
		{
			parsedMessage[key] = priKey[key];
		}
	}
...
function decodePri(message)
{
	...
	returnVal = {
		...
		version: parseInt(privalMatch[2])
	};
